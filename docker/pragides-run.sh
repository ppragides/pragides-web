...
if [ ! -e $PRAGIDES_WEB_DIR ]; then
	git clone --recursive $GIT_URL/pragides-web.git $PRAGIDES_WEB_DIR
fi
...
if [[ $GIT_PULL ]]; then
	...
	cd $PROJECT_ROOT/$PRAGIDES_WEB_DIR; git pull $GIT_URL/pragides-web.git
...
#if [[ ! -e "$PROJECT_ROOT/$PRAGIDES_WEB_DIR/app/node_modules" || $NPM_INSTALL ]]; then
#	echo Installing npm modules...
#	docker run --rm \
#		-v $PROJECT_ROOT/$PRAGIDES_WEB_DIR/app:/noderoot \
#		-e UID=1000 \
#		-e GID=1000 \
...
docker run -d \
	-p 81:80 \
	--restart=always \
	-v $PROJECT_ROOT/$PRAGIDES_WEB_DIR/webroot/:/var/www/pragides.ca/webroot \
	-v $PROJECT_ROOT/$PRAGIDES_WEB_DIR/etc/nginx/conf.d/pragides.conf:/etc/nginx/nginx.conf \
